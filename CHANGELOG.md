...
### Added
- **Autonomous privilege elevation for PostgreSQL backups (opt-in)**: databases can now configure a separate "elevation role" in the console (Databases → database → Tables & Storage Footprint → Privilege Elevation). When `pg_dump` fails with `permission denied for table/sequence/schema ...` and elevation is enabled, DBVault autonomously grants the backup role read access via the elevation role, retries the dump once, and **always revokes the grants afterwards** — including when the dump fails again, with 3 revoke retries and a loud security log if revocation cannot be confirmed. The denied-object list comes from a full privilege audit (tables + sequences + schemas, not just the parsed error); identifiers are sanitized and the elevation role must differ from the backup role. Without elevation configured, the manual remediation hint from 0.2.2 still applies.
...
### Fixed
- **PostgreSQL catalog audit for unprivileged roles**: Auditing now queries `pg_catalog.pg_class` and `pg_namespace` directly rather than `pg_tables` and `pg_sequences` views (which filter out zero-privilege objects). The audit reliably uncovers all unreadable tables, views, partitions, and sequences in one query.
- **Guaranteed privilege revocation**: Elevated privilege revocation now runs in a deferred cleanup handler using a detached context with a dedicated timeout. Revocation always executes even if the parent backup context timed out or was cancelled.
- **Quoted identifiers and SQL keywords**: Identifier parsing safely handles double-quoted table names, mixed-case names, and reserved keywords (such as `"user"` and `"order"`), and correctly formats schema-qualified objects.
- **Regex keyword extraction**: Corrected pattern matching so PostgreSQL error messages like `permission denied for relation users` or `permission denied for sequence ...` do not mistakenly capture SQL keywords (`"relation"`, `"sequence"`) as object names.
- **Schema Explorer Privilege Elevation card visibility**: The Privilege Elevation card in the web console remains visible and configurable even when initial catalog queries return zero tables due to restricted permissions.
- **Hyphenated role name quoting**: Remediation and elevation SQL safely quotes database roles with hyphens (e.g. `dbvault-worker`).
