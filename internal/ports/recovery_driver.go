...
type BaseBackupPlanRequest struct {
	Source     domain.Source
	Repository domain.Repository
	BackupType domain.PhysicalBackupType
}
type BaseBackupPlan struct {
...
type LogCollectionConfigurationRequest struct {
	Source   domain.Source
	Mode     string
	SlotName string
}
type LogCollectionConfiguration struct {
...
type ChainValidationRequest struct {
	SourceID    domain.SourceID
	BaseBackups []domain.PhysicalBackupSet
	Logs        []domain.TransactionLog
	Timelines   []domain.TimelineHistory
}
type PITRPlanRequest struct {
	SourceID    domain.SourceID
	Engine      domain.DatabaseEngine
	Target      domain.RecoveryTarget
	BaseBackups []domain.PhysicalBackupSet
	Logs        []domain.TransactionLog
	Windows     []domain.RecoveryWindow
}
type PITRExecutionRequest struct {
	Plan            domain.PITRPlan
...
type RecoveryDriverRegistry interface {
	Get(engine domain.DatabaseEngine) (RecoveryDriver, bool)
	List() []domain.RecoveryDriverDescriptor
}
