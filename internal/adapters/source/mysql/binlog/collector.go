...
func (c FileCollector) Push(ctx context.Context, filePath, name string) (domain.TransactionLog, error) {
	if !ValidName(name) {
		return domain.TransactionLog{}, domain.NewError(domain.ErrBinlogInvalid, "invalid binlog filename", nil)
	}
	...
	_, _ = f.Seek(0, 0)
	kind := domain.LogMySQLBinlog
	if c.Engine == domain.EngineMariaDB {
		kind = domain.LogMariaDBBinlog
	}
	key := string(c.SourceID) + "/binlog/" + c.ServerID + "/" + name
	if c.Store != nil {
		_, err = c.Store.Put(ctx, ports.PutObjectRequest{Key: key, Body: f, Size: n, ContentType: "application/octet-stream", IfNotExists: true})
		if err != nil {
			return domain.TransactionLog{}, err
		}
	}
